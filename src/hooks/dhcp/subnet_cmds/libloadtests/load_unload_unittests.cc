...
/// @brief Test fixture for testing loading and unloading the subnet cmds library
class SubnetCmdsLibLoadTest : public isc::test::LibLoadTest {
public:
    /// @brief Constructor
    SubnetCmdsLibLoadTest() : LibLoadTest(LIBDHCP_SUBNET_CMDS_SO) {
    }

    /// @brief Destructor
    virtual ~SubnetCmdsLibLoadTest() {
        unloadLibraries();
    }
};

// Simple V4 test that checks the library can be loaded and unloaded several times.
TEST_F(SubnetCmdsLibLoadTest, validLoad4) {
    validDaemonTest("kea-dhcp4");
}

// Simple V6 test that checks the library can be loaded and unloaded several times.
TEST_F(SubnetCmdsLibLoadTest, validLoad6) {
    validDaemonTest("kea-dhcp6", AF_INET6);
}

// Simple V6 test that checks the library cannot by loaded by invalid daemons.
TEST_F(SubnetCmdsLibLoadTest, invalidDaemonLoad) {
